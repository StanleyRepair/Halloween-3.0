...
const BASE_W=960,H=540,GROUND=430;
...
function mount(host){
  ...
  function finish(){
    ...
    if(isNew){best=score;localStorage.setItem(BEST_KEY,String(best));bestEl.textContent=String(best)}
    burst(player.x+player.w/2,player.y+player.h/2,'#ff7a2f',28);
    draw();
    card.querySelector('h3').textContent=isNew?'🏆 Nowy rekord!':'💀 Koniec biegu';
  ...
  function spawn(){
    ...
    const o={x:viewW+70,type,w:40,h:42,y:GROUND-42};
    ...
    const baseGap=type==='double'||type==='grave'?365:320;
    spawnDistance=rand(baseGap,baseGap+135)-difficulty*28;
  }
  ...
  function update(dt){
    ...
    for(const d of decor){d.x-=speed*.22*dt;if(d.x<-80)d.x+=Math.max(viewW,BASE_W)+300}
  ...
  function drawBackground(){
    const g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,'#10061a');g.addColorStop(.5,'#250b19');g.addColorStop(1,'#080507');ctx.fillStyle=g;ctx.fillRect(0,0,viewW,H);
    const moonX=Math.max(620,viewW-180);ctx.fillStyle='rgba(255,124,42,.13)';ctx.beginPath();ctx.arc(moonX,110,72,0,Math.PI*2);ctx.fill();ctx.fillStyle='rgba(255,207,134,.72)';ctx.beginPath();ctx.arc(moonX,110,48,0,Math.PI*2);ctx.fill();
    ctx.fillStyle='rgba(0,0,0,.34)';const citySpan=Math.max(1200,viewW+300);for(let i=0;i<Math.ceil(citySpan/120)+2;i++){const x=((i*137-distance*.05)%citySpan+citySpan)%citySpan-80;const h=randHash(i)*90+55;ctx.fillRect(x,GROUND-h,28,h);ctx.beginPath();ctx.moveTo(x-15,GROUND-h);ctx.lineTo(x+14,GROUND-h-45);ctx.lineTo(x+43,GROUND-h);ctx.fill()}
    ctx.fillStyle='rgba(123,41,24,.26)';const hillSpan=Math.max(1350,viewW+400);for(let i=0;i<Math.ceil(hillSpan/180)+2;i++){const x=((i*190-distance*.1)%hillSpan+hillSpan)%hillSpan-120;ctx.beginPath();ctx.arc(x,330,55,Math.PI,0);ctx.fill()}
  }
  function randHash(i){const x=Math.sin(i*999+17)*43758.5453;return x-Math.floor(x)}
  function drawGround(){
    ctx.fillStyle='#120909';ctx.fillRect(0,GROUND,viewW,H-GROUND);ctx.fillStyle='#8a321a';ctx.fillRect(0,GROUND,viewW,5);ctx.fillStyle='rgba(255,123,50,.14)';
    for(let x=(-distance*.6)%58;x<viewW;x+=58)ctx.fillRect(x,GROUND+30,32,4);
