...
const GAME_INFO={
  ...
  halloween_dash:{title:'Halloween Dash',icon:'🎃'}
};
...
function otherStatsMarkup(){
  const tiles=Array.isArray(data?.other_tiles)?data.other_tiles:[];
  const rows=tiles.length?tiles.map(t=>`<div class="analytics-other-row"><div><strong>${esc(t.title||'Kafelek')}</strong><small>${esc(t.id||'')}</small></div><div><b>${fmt(t.views)}</b><span>wejścia</span></div><div><b>${fmt(t.downloads)}</b><span>pobrania PDF</span></div></div>`).join(''):'<div class="analytics-other-empty">Brak wejść do kafelków Inne od ostatniego resetu.</div>';
  return `<div class="analytics-stat analytics-wide analytics-other">${resetBtn('other')}<div class="analytics-icon">☰</div><strong>${fmt(data?.other_views)} / ${fmt(data?.other_downloads)}</strong><span>Inne: wejścia w kafelki / pobrania PDF</span><small>Regulamin, menu, alergeny i pozostałe kafelki są liczone osobno.</small><button type="button" class="secondary analytics-other-toggle" data-other-stats-toggle>Pokaż szczegóły</button><div class="analytics-other-breakdown" data-other-stats-breakdown hidden>${rows}</div></div>`;
}
function render(){
  ...
  card.querySelector('[data-other-stats-toggle]')?.addEventListener('click',e=>{const box=card.querySelector('[data-other-stats-breakdown]');if(!box)return;box.hidden=!box.hidden;e.currentTarget.textContent=box.hidden?'Pokaż szczegóły':'Ukryj szczegóły'});
