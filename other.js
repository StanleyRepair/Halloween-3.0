...
async function getNotificationState(){
  const result={supported:'Notification'in window&&'serviceWorker'in navigator,permission:'unsupported',subscription:false,reg:null};
  if(!result.supported)return result;
  result.permission=Notification.permission;
  try{result.reg=await navigator.serviceWorker.ready;result.subscription=!!(await result.reg.pushManager?.getSubscription?.())}catch(e){console.warn('Notification diagnostics failed',e)}
  return result;
}
async function showNotifications(tile,push=true){
  ...
  const controls=[];
  if(state.permission==='granted'&&state.reg)controls.push('<button type="button" class="other-back" data-notification-test>🔔 Wyślij test</button>');
  if(state.permission==='default'||(state.permission==='granted'&&!state.subscription))controls.push('<button type="button" class="other-back" data-notification-news>Przejdź do Aktualności</button>');
...
view.addEventListener('click',async e=>{
  ...
  if(e.target.closest('.other-back')&&!e.target.closest('[data-notification-test],[data-notification-news],[data-notification-repair]')){goBack();return}
  ...
  if(e.target.closest('[data-notification-test]')){
    const out=detail.querySelector('[data-notification-result]');try{const reg=await navigator.serviceWorker.ready;await reg.showNotification('Halloween 3.0 🎃',{body:'To jest test powiadomień. Jeśli go widzisz, system pozwala je wyświetlać.',icon:'./icons/notification-news.svg?v=1',badge:'./icons/notification-badge.svg?v=1',tag:'h3-manual-notification-test',renotify:false});if(out)out.innerHTML='<strong>Test wysłany.</strong><br>Jeśli powiadomienie nie pojawiło się również w centrum powiadomień, wybierz „Napraw / instrukcja” i sprawdź ustawienia systemowe.'}catch(err){console.warn(err);if(out)out.textContent='Nie udało się wysłać testu. Sprawdź uprawnienia i spróbuj ponownie.'}return}
