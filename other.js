...
function tileMarkup(t,sub=false){const hasContent=t.content_type==='menu'?(t.children||[]).length:!!((t.body||'').trim()||t.image_path);return `<button type="button" class="${sub?'other-subtile':'other-tile'}" data-other-id="${t.id}"><span class="other-tile-icon">${esc(t.icon||'📄')}</span><span><strong>${esc(t.title)}</strong><small>${hasContent?(t.content_type==='menu'?'Otwórz podmenu':'Otwórz'):'JUŻ WKRÓTCE.. 🔧'}</small></span><span class="other-chevron">›</span></button>`}
...
function showTile(tile){list.hidden=true;detail.hidden=false;let inner='';if(tile.content_type==='menu'){inner=(tile.children||[]).length?`<div class="other-submenu">${tile.children.map(c=>tileMarkup(c,true)).join('')}</div>`:'<div class="other-soon">JUŻ WKRÓTCE.. 🔧</div>'}else if(tile.content_type==='image'){inner=tile.image_path?`<img class="other-detail-image" src="${imageUrl(tile.image_path)}" alt="${esc(tile.title)}">`:'<div class="other-soon">JUŻ WKRÓTCE.. 🔧</div>'}else{inner=(tile.body||'').trim()?`<div class="other-detail-body">${esc(tile.body)}</div>`:'<div class="other-soon">JUŻ WKRÓTCE.. 🔧</div>'}detail.innerHTML=`<button type="button" class="other-back">← Wróć</button><div class="other-detail"><div class="other-detail-head"><span class="other-detail-icon">${esc(tile.icon||'📄')}</span><h2>${esc(tile.title)}</h2></div>${inner}</div>`}
