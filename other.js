...
const list=view.querySelector('#otherTiles'),detail=view.querySelector('#otherDetail');let data=[],navStack=[],creepyAssetsPromise=null,candleAssetsPromise=null;
const GAMES_ID='__games__',CREEPY_ID='__creepy_pumpkin__',CANDLE_ID='__candle_game__',NOTIF_ID='__notifications__';
const creepyTile={id:CREEPY_ID,title:'Creepy Pumpkin',icon:'🎃',body:'',image_path:null,children:[],builtin_creepy_game:true};
const candleTile={id:CANDLE_ID,title:'Zgaś świece',icon:'🕯️',body:'',image_path:null,children:[],builtin_candle_game:true};
const gamesTile={id:GAMES_ID,title:'Gry',icon:'🎮',body:'',image_path:null,children:[creepyTile,candleTile],builtin_games:true};
...
function stopGames(){window.CreepyPumpkinGame?.unmount?.();window.H3CandleGame?.unmount?.()}
function hasOwnContent(t){return !!((t.body||'').trim()||t.image_path||t.builtin_creepy_game||t.builtin_candle_game||t.builtin_games||t.builtin_notifications)}
...
function ensureCreepyAssets(){if(window.CreepyPumpkinGame)return Promise.resolve();if(creepyAssetsPromise)return creepyAssetsPromise;creepyAssetsPromise=new Promise((resolve,reject)=>{if(!document.querySelector('link[data-creepy-pumpkin]')){const l=document.createElement('link');l.rel='stylesheet';l.href='creepy-pumpkin.css?v=2';l.dataset.creepyPumpkin='1';document.head.appendChild(l)}const s=document.createElement('script');s.src='creepy-pumpkin.js?v=3';s.async=true;s.onload=resolve;s.onerror=reject;document.head.appendChild(s)});return creepyAssetsPromise}
function ensureCandleAssets(){if(window.H3CandleGame)return Promise.resolve();if(candleAssetsPromise)return candleAssetsPromise;candleAssetsPromise=new Promise((resolve,reject)=>{if(!document.querySelector('link[data-candle-game]')){const l=document.createElement('link');l.rel='stylesheet';l.href='candle-game.css?v=1';l.dataset.candleGame='1';document.head.appendChild(l)}const s=document.createElement('script');s.src='candle-game.js?v=1';s.async=true;s.onload=resolve;s.onerror=reject;document.head.appendChild(s)});return candleAssetsPromise}
function renderList(){stopGames();navStack=[];detail.dataset.currentId='';detail.hidden=true;list.hidden=false;list.innerHTML=rootTiles().map(t=>tileMarkup(t)).join('')||'<div class="other-soon">JUŻ WKRÓTCE.. 🔧</div>'}
function findTile(id){if(id===GAMES_ID)return gamesTile;if(id===CREEPY_ID)return creepyTile;if(id===CANDLE_ID)return candleTile;if(id===NOTIF_ID)return notificationTile;for(const r of data){if(r.id===id)return r;const c=(r.children||[]).find(x=>x.id===id);if(c)return c}return null}
...
async function showNotifications(tile,push=true){
  stopGames();if(push&&detail.dataset.currentId)navStack.push(detail.dataset.currentId);list.hidden=true;detail.hidden=false;detail.dataset.currentId=tile.id;
...
async function showCreepyGame(tile,push=true){stopGames();if(push&&detail.dataset.currentId)navStack.push(detail.dataset.currentId);list.hidden=true;detail.hidden=false;detail.dataset.currentId=tile.id;detail.innerHTML=`<button type="button" class="other-back">← Wróć</button><div class="other-detail creepy-game-detail"><div class="other-detail-head creepy-game-head"><span class="other-detail-icon">🎃</span><h2>Creepy Pumpkin</h2><button type="button" class="creepy-fullscreen-button" aria-label="Włącz pełny ekran">⛶</button></div><div id="creepyPumpkinHost"><div class="other-soon">Uruchamianie gry...</div></div></div>`;try{await ensureCreepyAssets();if(detail.dataset.currentId!==CREEPY_ID)return;window.CreepyPumpkinGame?.mount?.(detail.querySelector('#creepyPumpkinHost'));detail.querySelector('.creepy-fullscreen-button')?.addEventListener('click',()=>window.CreepyPumpkinGame?.toggleFullscreen?.())}catch(e){console.warn(e);detail.querySelector('#creepyPumpkinHost').innerHTML='<div class="other-soon">Nie udało się uruchomić gry.</div>'}}
async function showCandleGame(tile,push=true){stopGames();if(push&&detail.dataset.currentId)navStack.push(detail.dataset.currentId);list.hidden=true;detail.hidden=false;detail.dataset.currentId=tile.id;detail.innerHTML=`<button type="button" class="other-back">← Wróć</button><div class="other-detail"><div class="other-detail-head"><span class="other-detail-icon">🕯️</span><h2>Zgaś świece</h2></div><div id="candleGameHost"><div class="other-soon">Uruchamianie gry...</div></div></div>`;try{await ensureCandleAssets();if(detail.dataset.currentId!==CANDLE_ID)return;window.H3CandleGame?.mount?.(detail.querySelector('#candleGameHost'))}catch(e){console.warn(e);detail.querySelector('#candleGameHost').innerHTML='<div class="other-soon">Nie udało się uruchomić gry.</div>'}}
function showTile(tile,push=true){stopGames();if(tile.builtin_notifications)return showNotifications(tile,push);if(tile.builtin_creepy_game)return showCreepyGame(tile,push);if(tile.builtin_candle_game)return showCandleGame(tile,push);if(push&&detail.dataset.currentId)navStack.push(detail.dataset.currentId);list.hidden=true;detail.hidden=false;detail.dataset.currentId=tile.id;const parts=[];if((tile.body||'').trim())parts.push(`<div class="other-detail-body">${esc(tile.body)}</div>`);if(tile.image_path)parts.push(`<img class="other-detail-image" src="${imageUrl(tile.image_path)}" alt="${esc(tile.title)}">`);if((tile.children||[]).length)parts.push(`<div class="other-submenu">${tile.children.map(c=>tileMarkup(c,true)).join('')}</div>`);detail.innerHTML=`<button type="button" class="other-back">← Wróć</button><div class="other-detail"><div class="other-detail-head"><span class="other-detail-icon">${esc(tile.icon||'📄')}</span><h2>${esc(tile.title)}</h2></div>${parts.length?parts.join(''):'<div class="other-soon">JUŻ WKRÓTCE.. 🔧</div>'}</div>`}
function goBack(){stopGames();if(navStack.length){const prev=findTile(navStack.pop());if(prev)return showTile(prev,false)}renderList()}
async function loadOther(){stopGames();list.innerHTML='<div class="other-soon">Ładowanie...</div>';try{const{data:d,error}=await sb.rpc('get_other_tiles');if(error)throw error;data=d||[];renderList()}catch(e){console.warn(e);data=[];renderList()}}
...
document.querySelector('.nav-item[data-tab="other"]')?.addEventListener('click',loadOther);document.querySelectorAll('.nav-item:not([data-tab="other"])').forEach(b=>b.addEventListener('click',stopGames));if(location.hash==='#other')setTimeout(()=>document.querySelector('.nav-item[data-tab="other"]')?.click(),0);
})();
