...
function mount(host){
  ...
  let H=900,scale=1,best=Number(localStorage.getItem(BEST)||0),score=0,wave=1,nextWave=1,rows=[],spawnClock=0,bullets=[],parts=[],damage=1,delay=.24,multi=1,fire=0,destroyed=0,x=270,target=270,running=false,raf=0,last=0,drag=false,full=false,pushed=false,ro=null;
  ...
  function shipPower(){const rate=.24/Math.max(.075,delay),spread=1+(multi-1)*.45;return Math.max(1,damage*rate*spread)}
  ...
  function reset(){score=0;wave=1;nextWave=1;rows=[];spawnClock=0;bullets=[];parts=[];damage=1;delay=.24;multi=1;fire=0;destroyed=0;x=target=270;addRow();hud();draw()}
      ...
      for(const r of hitRows){
        ...
        for(const b of r.b){
          ...
          if(Math.abs(p.x-b.x)<70){
            ...
            if(b.hp<=0){b.broken=true;destroyed++;b.drop=destroyed%4===0;if(!b.drop)b.taken=true;score+=38+r.w*3;burst(b.x,r.y,b.drop?'#ffc56c':'#ff6b27',b.drop?15:10);try{navigator.vibrate?.(b.drop?[12,20,22]:12)}catch{}}
            break;
          }
        }
        if(hit)break;
