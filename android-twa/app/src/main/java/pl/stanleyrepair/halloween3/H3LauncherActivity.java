...
public class H3LauncherActivity extends Activity {
    ...
    private final CustomTabsCallback callback = new CustomTabsCallback() {
        @Override public void onRelationshipValidationResult(int relation, @NonNull Uri requestedOrigin, boolean result, @Nullable Bundle extras) {
        ...
        @Override public void onPostMessage(@NonNull String message, @Nullable Bundle extras) {
            ...
            Uri updateUri = new Uri.Builder().scheme("halloween3").authority("update")
                    .appendQueryParameter("url", parts[1]).appendQueryParameter("version", parts[2]).build();
    ...
    private Uri launchUrl() {
        ...
        Uri base = getIntent()!=null && getIntent().getData()!=null && "https".equals(getIntent().getData().getScheme()) ? getIntent().getData() : Uri.parse("https://stanleyrepair.github.io/Halloween-3.0/");
