...
begin
  ...
  with discovered as (
    ...
    select game_key from (values ('creepy'),('candle'),('halloween_match'),('dark_ritual'),('halloween_dash'),('pumpkin_blaster')) v(game_key)
    union
    ...
    'games',coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'reset_at',g.reset_at
        ) order by case g.game_key when 'creepy' then 1 when 'candle' then 2 when 'halloween_match' then 3 when 'dark_ritual' then 4 when 'halloween_dash' then 5 when 'pumpkin_blaster' then 6 else 99 end,g.game_key
      ) from game_stats g
    ),'[]'::jsonb)
  ) into result;
  return result;
