...
let deferredPrompt=null,waiters=[],forcedPlatform=null,safariAutoTimer=null;
...
function updateIosInAppNotice(){
  ...
  if(!notice){notice=document.createElement('div');notice.id='iosInAppNotice';notice.className='ios-inapp-notice';notice.setAttribute('role','note');notice.innerHTML='<div class="ios-inapp-notice-copy"><strong>Otwieram Safari…</strong><span>Jeśli Safari nie otworzy się automatycznie, użyj przycisku <b>Otwórz w Safari</b> poniżej.</span></div><span class="ios-inapp-pointer" aria-hidden="true">↗</span>';document.body.appendChild(notice)}
...
function configureBrowserCard(){
  ...
  if(activePlatform()==='ios'&&isIOS&&!isSafariIOS){
    ...
    if(kicker)kicker.textContent='OTWÓRZ W SAFARI';
    if(heading)heading.textContent='Instalacja wymaga Safari';
    if(copy)copy.textContent='Próbuję otworzyć tę stronę w Safari. Jeśli nic się nie stanie, stuknij przycisk poniżej.';
    chromeBtn.textContent='OTWÓRZ W SAFARI';
    if(note)note.textContent='Po przejściu do Safari pokaże się krótka instrukcja instalacji.';
    return;
  }
  if(mark)mark.textContent='🌐';
...
function openSafari(auto=false){const target=safariTarget();if(!isIOS||isSafariIOS||!target)return false;window.H3Analytics?.track?.(auto?'install_open_safari_auto':'install_open_safari_click',{source:isInAppBrowser?'in_app_browser':'ios_browser'});try{location.href=target;return true}catch(e){console.warn('Safari open failed',e);return false}}
function attemptSafariAuto(){if(!isIOS||isSafariIOS||standalone)return;let done=false;try{done=sessionStorage.getItem('h3_safari_auto_attempted')==='1';if(!done)sessionStorage.setItem('h3_safari_auto_attempted','1')}catch{}if(done)return;clearTimeout(safariAutoTimer);safariAutoTimer=setTimeout(()=>openSafari(true),280)}
function showInstallChoice(){const platform=activePlatform();if(platform==='ios'){if(isIOS&&!isSafariIOS&&!standalone){show(browserCard);attemptSafariAuto()}else{show(iosCard);window.H3Analytics?.track?.('install_landing_ios_instructions',{source:'install_page',manual:forcedPlatform==='ios',ios_major:detectedIOSVersion?.major||null})}return}if(platform==='android'){if(isChrome)show(installCard);else show(browserCard);return}if(isChrome){show(installCard);return}show(browserCard)}
