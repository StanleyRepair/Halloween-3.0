...
createBtn.onclick=async function(e){
 const toggle=document.getElementById('newNewsPush');const wantsPush=!!toggle?.checked;const title=document.getElementById('newNewsTitle')?.value?.trim()||'';const body=document.getElementById('newNewsBody')?.value?.trim()||'';
 await original?.call(this,e);const created=!!title&&!(document.getElementById('newNewsTitle')?.value?.trim());if(!created)return;let pushSent=false;
